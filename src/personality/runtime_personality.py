...
def build_runtime_personality(personality: dict | None, memory: dict | None) -> str:
    ...
    if isinstance(memory, dict):
        preferences = _collect_text_list(memory.get("user_preferences"))
        emotional_patterns = _collect_text_list(memory.get("emotional_patterns"))
        recurring_topics = _collect_text_list(memory.get("recurring_topics"))
